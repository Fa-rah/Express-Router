...
router.post('/checKlogin', (req, res) => {
  ...
  if (token)
    jwt.verify(token, 'publickKey', (err, decoded) => {
      if (err) {
        return res.status(500).send({ message: 'Payload is unvalidate!' });
      }
      res.status(200).send({ message: 'You are logged in!', decoded });

      return res.status(401).send({ message: 'You have to log in!' });
    });
});
  ...
  User.findOne({ email: req.body.email })
    ...
    .then((user) => {
      bcrypt.compare(req.body.password, user.password, (err, result) => {
        if (err) {
          return res.status(401).send('The password is not valid!');
        }
        if (result) {
